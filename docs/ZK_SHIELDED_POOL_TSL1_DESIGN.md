...
## 15. Coordinator and protocol (2026-09-22)

**BUILT 2026-09-22** (OpenSpec change `tsl1-sp-coordinator`): the TSL1_SP coordinator in `lib/src/shielded_pool/shielded_coordinator.dart`, the wallet-to-coordinator messages in `pool_protocol.dart`, both exported, and the legacy coordinator, its chain reader and `bin/pool_coordinator.dart` deleted with their tests. The legacy tool and its script tests stay.

**The round as the coordinator builds it.** `ShieldedCoordinator` holds a `ShieldedLedger`, a pending round, and the pool's owner key. Closing a round is: drop any accepted transfer whose anchor has left the ring or whose note a round published meanwhile spent (an expired reply each); ask the funding source for Y's output, whose size is fixed for a pool, before anything is proved; pad to the plan's size from the padding stock, proving any shortfall; copy the ledger's two trees and advance them with the aggregation's own `subtreeLeavesOf`; aggregate through the prover pool and build the root unlock; advance the header; build Y_{N+1}; build the round and the witness once with placeholder funding to size them, ask for their outputs at the priced fee, and build them again with the exact fee and the real outpoints (the witness's funding outpoint is locked into the round's PP2, so the round is built last of the two dry runs and first of the two real ones); apply the three to the coordinator's own ledger with the reader's checks; hand them and the ledger's snapshot to the store; publish Y, the round, the witness. A failure anywhere before the apply returns the transfers to the front of the pending round and the padding to the stock, and records a `RoundFailure` naming the stage. Rounds are built one at a time; intake keeps running into the next pending round meanwhile.

**Intake order.** A submission is checked in cost order and refused at the first failure with a numbered `RefusalReason`: the submission decodes (128 KB bound) and its transfer decodes (100 KB); the transfer is consistent with itself (`ShieldedTransfer.refusal`: outHash, withdrawal, bundle commitments, deposit shape); its anchor is in the ring, unless nothing real is spent; its real nullifiers are neither in the tree nor claimed by the pending or an in-flight round; a deposit carries the covenant transaction, whose named output is a covenant for the tip's PP3, refundable at least the configured margin beyond the chain height the server last set, with the transfer's receipt, not already backed by a pending transfer, and with a receipt slot left; the pending and in-flight withdrawals plus this one stay within the balance; and last the proof, at 20 ms. The coordinator counts its verifications, which is how the tests show that a bad proof costs one and every cheaper refusal none. A submission that does not decode past its id is refused as malformed; one with no readable id is dropped.

**Deposits by covenant.** A covenant names PP3_N, so only the round that spends PP3_N can take it. A covenant naming any other PP3 (an older round's, or another pool's) is refused with the live PP3 in the sentence, and so is one naming the live PP3 while a round that spends it is in flight: the depositor waits for the announcement and deposits against the new PP3. That window is the round's build time, about four minutes at production on the GPU.

**Funding.** `CoordinatorFunding.output(minValue)` returns a P2PKH output of at least that value with its signer; the coordinator asks for three a round and checks each value before spending it. Y and the witness have no change output, so it asks for exactly the fee plus their satoshi outputs; the round has change, so it asks for the fee plus the dust floor. The fee is the size at the configured rate, never below the floor, with 8 bytes added to a dry-built size because a DER signature varies by a byte. The tool gained `ownerSigner` on the round and `fundingSigner` on the witness so a funding output may belong to a key other than the pool's owner. The localnet harness's funding source spends its change to mint one exact output per request and mines it, as a server's wallet will.

**The pre-publish apply and the store.** The coordinator's ledger applies the round it built before anyone else sees it, so the one failure the pool core could not prevent, a transfer whose bundle names another commitment and strands every reader, is caught by the party that can still stop it. The store is called after the apply with the snapshot, before the first publish, so a crash between the round's publish and the witness's leaves a recoverable witness rather than a frozen pool. Recovery (`ShieldedCoordinator.recover`) restores a snapshot or opens from genesis, applies the triples since through the chain reader, and refuses to run when a triple does not apply or the chain ends at a round other than the store's last, naming the tip and the triple.

**The protocol's four messages.** Each begins with a version byte and a kind byte, then fields with explicit little-endian lengths, so equal messages give identical bytes. A submission: a 16-byte id the wallet draws at random, the transfer bytes behind a 4-byte length, a deposit transaction behind a 2-byte length (0 for none, at most 4 KB). A reply: the id, an outcome byte, then for accepted the round number, for refused a 2-byte reason number and a sentence, for expired a sentence. A descriptor: the network, the txids of the issuance, witness 0 and Y_0, the arities, the nullifier level, the receipt slots, and the seven spend parameters. An announcement: the round number, the 236-byte header, and the txids of the round, its witness and the slot it pins. Decoding is bounded before reading (128 KB for a submission, 4 KB otherwise), every length is checked against the bytes left and its field's own bound before allocation, and every failure is a `ProtocolRefusal` naming the field. A fuzz of 10,000 random strings, 10,000 single-byte mutations and every 4-byte run of zeros and ones per kind finds no other outcome; with the decoder's catch-all disabled it still passes, so the explicit bounds carry it, and removing the reader's bytes-remaining check makes it fail. Nothing in a descriptor or an announcement is trusted: a wallet opens a ledger from the descriptor's txids and applies the announced triples, and `PoolAnnouncement.disagreement` names a header or a number the round's own PP1 does not carry.

**What a coordinator keeps.** Per accepted transfer, the transfer as submitted, its covenant transaction and the submission id, until the round is published; no wallet key, no note, nothing after. The pending round is not persisted: a restart loses it, and a wallet that does not find its notes in the announced rounds resubmits.

### 15.1 Measured

All on the coordinator's machine (Apple M3 Pro, 12 cores), one core unless stated.

**The protocol's sizes**, `tool/scratch/protocol_size_probe.dart`: a production deposit submission (a transfer with two hybrid bundles, its 1,512 B covenant transaction and a 16-byte id) encodes to 68,972 B against the 128 KB bound: 67,436 B of transfer, 1,512 B of deposit transaction, 24 B of framing. The test chain's deposit submission is 16,348 B. A descriptor is 113 B for the test chain and 115 B for the production plan, an announcement 338 B, and a reply 23 B accepted, 84 B refused with a sentence, 71 B expired, all against the 4 KB bound. Decoding the production submission takes 0.02 ms, decoding its transfer and deposit transaction 0.45 ms, and the transfer's self-consistency checks 0.06 ms: 0.53 ms in all against the 5 ms bound, before any proof is verified.

**The chain mined through the coordinator**, `POOL_LOCALNET=1 POOL_COORDINATOR=1 dart test test/pool_localnet_test.dart` (test chain through ARC, production through node RPC with `POOL_BROADCAST=rpc POOL_PRODUCTION=1 STARK_KERNELS_GPU=1`). The coordinator, funded by one exact node-mined output per request and publishing through the node, took the fixture's transfers and the deposit and mined both rounds at both scales; a reader given its three transactions a round reaches its ledger, snapshot for snapshot. At test parameters a round is 10.9 s and 11.5 s from close to mined, of which 3.8 s and 4.1 s is the node mining three blocks. At production the two rounds are 261 s and 258 s from close to mined on the GPU: 248 s and 244 s of aggregation, and the coordinator's own work 13 s a round (funding 0.3 s, trees 0.2 s, Y 0.6 s, the two dry builds 3.4 s, the round 2.7 s, the witness 0.6 s, the apply 0.7 s, the store 0.7 s, publishing 4 s). The transactions are the sizes the hand-built chain had: Y 1,783,001 B, round 1 395,698 B, witness 1 2,202,099 B, round 2 395,115 B, witness 2 2,529,395 B, each at 1.00 sat/kB, and the node validated the rounds in 925 and 930 ms. Taking in 256 production transfers took 5.4 s, 21 ms each. The transfer cache the run writes (`prod_proofs.json.transfers.json`, 65 MB) is what the probe below reads.
...
## 20. The grind digest is the transcript state (2026-09-26)

An outside adversarial review asked, among other things, for a written security claim. Writing it (`docs/SECURITY_CLAIM.md`) found deviation D1: the grinding nonce never reached the state the query indices are squeezed from. Every implementation agreed on this, so no test could see it. The verifier checked that H(S ‖ nonce) had its low bits zero and then squeezed the indices from S, not from the hash; the AIR even saved `stateBeforeGrind` and restored it. So the indices were a function of S alone. Anyone wanting a different set of queries changed a cheap absorbed value (a final-polynomial coefficient will do), read the new indices for one hash, kept only the states whose queries landed well, and ground once. The grind was a fixed 2^16 cost paid once, not a multiplier on every attempt, and its bits counted for nothing. The honest figure was 88 bits conjectured / 44 proven for the system (the spend level is the minimum), against the ~104 / ~106 the comments carried.

**The fix** (change `grind-binds-queries`) is one line of protocol: on success the grind digest becomes the transcript state, and the indices are squeezed from it. In the SHA-256 flavour `TranscriptRef.checkGrinding` sets `state = h`; in the Poseidon2 flavour `Poseidon2Transcript.checkGrinding` keeps the whole 8-lane digest; the script emitter's `emitGrindingCheck` rolls `ts` in, hashes, and keeps the hash as the new `ts` (a DUP feeds the zero-prefix check); the verifier AIR continues from the grind period instead of restoring the period before it. The nonce search is untouched, so the native kernels are untouched; the Dart `grind` wrappers run the check on the nonce the kernel returns so their state advances too. The reference prover's second `checkGrinding` after `grind`, an assertion, went, since it would now advance the state twice.

**Cost: none.** The aggregated verifier template is 504,184 script bytes before and after, the k8 direct-slot template 65,443 before and after: the PICK became a ROLL, the DUP was paid for by the old `ts` no longer needing its drop. The AIR has the same periods; the squeeze after the grind chains from it where it used to restart from a saved digest.

**The attack, as a test.** Take a valid proof and replace its nonce with the next nonce that also meets the target. Before the fix every verifier accepted it. Now the reference verifier refuses it at `query 0 index`, the script fails at its index comparison, and the verifier AIR has no satisfying witness (constraint 57 at the first index squeeze). `test/grind_binds_queries_test.dart` (both flavours, transcript and proof level), `test/stark_verifier_pieces_test.dart` (script), `test/verifier_air_test.dart` (AIR). The three existing grind tests that re-checked a nonce after grinding were adjusted to work on copies of the pre-grind state, since a check now moves the state.

**Compatibility.** Every proof, exported template and mined chain made before this is invalid against this code, and the two localnet chains under `../localnet` must be re-mined when next needed. There is no proof-format version to move and the wire protocol did not change, so the incompatibility rides on the package: the next release is 3.0.0, and the sibling repos (pool-coordinator, libcloak, cloak-cli) upgrade together. None is on mainnet. The `stark-prover` spec now states security in two columns with their assumptions and counts the Poseidon2 grind at its real 14 bits; with the grind counted the system is 102 bits conjectured / 58 proven (spend level), the root slot 106 / 61.

What this leaves open is the proven column: nothing reaches 64 proven bits, and whether that column should carry a requirement is a decision recorded as D3 in the claim document.

## 21. The round state machine, model-checked (2026-09-26)

Sections 8 and 9 argue by hand about who can spend what and when: PP3 supplies the timing and PP1 the sight (5.2), V cannot run in the witness (8.2), Y_N exists before round N so V cannot name it (5.5). Every flaw found in this design so far was an argument of that shape, and the review asked for the attack table to be extended by someone other than its author. `formal/tla/PoolRounds.tla` is that argument as a TLA+ model: transactions are actions, each script is a predicate on the transaction spending it, and anyone builds anything the scripts accept. Bytes and parsing stay with the interpreter tests; the STARK is assumed sound (a proof exists exactly when its statement is true of the header it is proved against, wherever that header came from).

Each protection this design added is a constant of the model, so switching one off reproduces the attack it closed. With everything on and no burn path, the four safety invariants (nobody takes out more than they paid in; a pool opened empty owes its depositor exactly what they paid in; a pinned, certified slot is spent only by a round or the coordinator; no deposit is taken) hold over 240,933 distinct states at two rounds and one deposit. Switching off genesis certification, V's signer, the burn removal, both program pins, or the depositor's lineage check each produces a counterexample, and the counterexamples are the attacks of 11.15 vector 5 and vector 1, 5.6, 5.4 and 16 respectively (`formal/tla/README.md` has the table).

Two things it said that this document had not. Vector 5 is shorter than 11.15 describes: with a decoy V_0 the coordinator need not fabricate notes and wait, since a round whose V is a decoy has no proof constraining its outputs and can pay the round's own deposit straight to a stranger. And the two program pins of 5.5 step 6 are each sufficient alone: with only PP3's forward covenant or only V's constant programs, no theft is found. The anchor (4.1) is confirmed as a liveness measure: without it nothing is taken, the coordinator can only freeze.

What the model does not yet do: liveness (that an honest coordinator can always reach the next witnessed round), time (refund heights against round assembly), more than one depositor, and the chain reader's side of section 16.

**Liveness, added the same day.** "An honest coordinator can always advance" cannot be a fairness-based liveness property here, because an outsider who acts forever (re-mining decoy slots, say) is a legal behaviour and would defeat any such property without saying anything about the design. It is checked instead as a possibility, a state property: in every reachable state the pool is finished or the honest coordinator's next step is enabled, where that step is to mine or re-mine the slot the next round needs, issue on the empty tree, build a round receiving every open deposit and paying nobody, or witness the round just built. With every protection on it holds (1,941 states, the coordinator being restricted to honest moves); with V's signer removed it fails in 438 states with the trace of 11.15 vector 1: the outsider spends the pinned V_0 with a copied proof and, once witness 0 is mined, the coordinator has no move left. One relaxation was needed: a slot nobody has pinned may be replaced by another Y_n, which is what vectors 2 and 3 already say. Without it an outsider's decoy at an index would block the honest coordinator in the model, an artefact of indexing slots by round rather than by txid.
