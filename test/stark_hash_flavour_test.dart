...
void main() {
  ...
  test('Poseidon2 transcript and leaves are deterministic and flavour-specific', () {
    ...
    expect(t1.checkGrinding(nonce, 1), isTrue);
    expect(t1.checkGrinding([nonce[0] + 1], 1), isFalse);
  ...
  test('Poseidon2-flavour spend proofs verify; tampering and wrong publics are caught', () {
    ...
    final bad = StarkProof(
        ...
        queries: [
          QueryProof(
              index: q.index, compLeaf: [...q.compLeaf]..[0] ^= 1, compPath: q.compPath, yAInv: q.yAInv,
