...
- [x] 1.1 Record the per-stage baseline in a new design-doc section "Node stages
...
- [x] 2.1 Extend `sk_deep_quotients` to take a second constant set and produce
...
- [x] 2.2 Use the fused call in `StarkProver`, check the byte-identity suites
...
- [x] 3.1 Split the composition-values lap into evaluation and constraint-program
...
- [x] 4.1 Add a kernel entry point that searches nonce blocks in parallel and
...
- [x] 4.2 Test that the kernel nonce equals the Dart loop's at a grind size
...
- [x] 5.1 Add intra-column parallelism to `evaluate` and `interpolate` for large
  ...
  off in `tool/scratch/kernel_bench.dart`. Outcome: built, measured and
  reverted. The interpolation split is 2.1x slower per column; the extension
  split is 9% faster per column but no commitment in a round is large enough
  below the threshold for the round to see it. The code is not kept; the
  numbers and the reason are in design.md and the design doc.

## 6. Measurement

- [x] 6.1 Re-run the round and record the new stage totals and round time in the
