...
- [ ] 5.2 Implement versioned ledger snapshots (version byte, header, tip transactions, leaves in order, nullifiers in order; restore replays and checks the roots against the header); verify "Restart" (snapshot after round 1, restore, apply round 2, same header), "Edited snapshot" (one leaf altered is refused), and that an unknown snapshot version is refused.
- [ ] 5.3 Verify pool-ledger "Two independent readers": two ledgers built separately from the test chain's transactions serialise to byte-identical snapshots.
- [ ] 5.4 Make every parse of round, witness and slot input end in a named refusal that leaves the ledger unchanged; verify pool-ledger "Truncated witness", and a mutation test applying 1,000 single-byte mutations of round 1 to fresh genesis ledgers asserting each outcome is applied or a named refusal ("Mutated round").
- [ ] 5.5 Measure in `tool/scratch/ledger_apply_probe.dart` against the pool-ledger bounds: applying one production round from the localnet production dump ("Reader keeps up", under 5 s on one core) and restoring a snapshot of 1,000 synthetic production rounds ("Restore time", 512,000 leaves, under 10 s; if over, add the frontier and node map to the snapshot and update design.md); record both, with the machine, in the design doc.
...
- [ ] 6.3 Measure scanning one production round for one diversifier (512 hybrid bundles) in `tool/scratch/scan_cost_probe.dart` against pool-ledger "Scanner keeps up" (under 5 s on one core); record it, with the machine, in the design doc. If it fails, stop and raise it: the remedies in design.md change the bundle format.
- [ ] 6.4 Verify pool-ledger "Offline scan": write the test chain's rounds and witnesses to files, scan them with no network access, and assert the same notes are found as from the in-memory chain.
...
- [ ] 7.3 Write the dated "pool core" section in `docs/ZK_SHIELDED_POOL_TSL1_DESIGN.md`: the transfer and its wire format, why commitments come from bundles and what a mismatched bundle does, the reader's inputs, snapshots and scanning, with the numbers from 1.7, 2.3, 5.5, 6.3 and 7.2.
