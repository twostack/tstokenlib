...
## Purpose
Runs a TSL1_SP pool over time: accepts submissions from wallets as hostile input, closes rounds on a schedule, keeps the stock and caches warm between rounds, publishes only rounds its own ledger accepts, and recovers its ledger from a snapshot and the chain after a restart.
...
- **THEN** it is accepted in under 50 ms (measured 2026-09-22, `tool/scratch/coordinator_cost_probe.dart`: 18 ms mean, 36 ms worst over 256 production transfers)

#### Scenario: Round cost
- **WHEN** a production round of the localnet run is closed
- **THEN** Y, the round and the witness are built in under 10 minutes (measured 2026-09-22 on the localnet production chain: 254 s with the GPU, 341 s without, the coordinator's own work about 9 s of either)

#### Scenario: Recovery cost
- **WHEN** a coordinator restores a 1,000-round snapshot and applies the two production rounds
- **THEN** it is ready in under 30 s (measured 2026-09-22: 9.7 s to restore plus 1.7 s for the two rounds)
