...
For every transfer the script SHALL check that its lanes are canonical, that its anchor is in the ring unless neither input is real, that the signed public amount moves the vault only for BSV, that a mint or gated transfer carries a valid issuer authorisation (Rabin signature over the lanes), that each real input's nullifier is inserted into the sorted nullifier set (non-membership then insertion, both proved by Merkle paths) and that SHA256 of its extra outputs equals its outHash lanes.
...
- **WHEN** a transfer's real flags are both zero and its anchor is not in the ring
...
The state script and the round's unlocking data SHALL keep every script under 1,000,000 ops and the transaction under 10 MB; at 256 transfers the state script binds at about 290 transfers per round.

#### Scenario: Round at plan size
- **WHEN** a 256-transfer aggregated round is assembled
- **THEN** the root verifier slot is about 2.2 MB and 883,000 ops and the transaction under 10 MB
