...
/// output index parameters for the PP5 and PP2 outputs.
class PP2FtLockBuilder extends LockingScriptBuilder {
  ...
  /// [_pp5OutputIndex] - Output index of the PP5 fungible token output.
  /// [_pp2OutputIndex] - Output index of the PP2-FT output.
  PP2FtLockBuilder(this._fundingOutpoint, this._witnessChangePKH, this._changeAmount, this._ownerPKH, this._pp5OutputIndex, this._pp2OutputIndex) {
  ...
  SVScript getScriptPubkey() {
    ...
    var scriptHex = template
        ...
        .replaceFirst("<pp5OutputIndex>", ScriptBuilder().number(_pp5OutputIndex).build().toHex())
  ...
  /// The output index of the PP5 fungible token output.
  int get pp5OutputIndex => _pp5OutputIndex;
  ...
  void parse(SVScript script) {
    ...
    // Read pp5OutputIndex (script number encoding)
    var pp5Byte = paramBytes[offset];
    if (pp5Byte == 0x00) {
      _pp5OutputIndex = 0;
    } else if (pp5Byte >= 0x51 && pp5Byte <= 0x60) {
      _pp5OutputIndex = pp5Byte - 0x50;
    } else {
      var numLen = pp5Byte;
      offset++;
      var numBytes = paramBytes.sublist(offset, offset + numLen);
      _pp5OutputIndex = castToBigInt(numBytes, true).toInt();
      offset += numLen;
    }
    offset++;
