...
///
/// The header ([PP1SpHeader], 226 bytes of pushes) is followed by the body
/// [PP1SpScriptGen.body], which depends on the STARK parameters and the
...
class PP1SpLockBuilder extends LockingScriptBuilder {
  ...
  PP1SpLockBuilder(PP1SpScriptGen gen, PP1SpHeader header)
      : _gen = gen,
        _header = header;

  PP1SpLockBuilder.fromScript(SVScript script) : super.fromScript(script);
  ...
  void parse(SVScript script) {
    ...
    if (buf.length < PP1SpHeader.bytesTotal) {
      throw ScriptException(ScriptError.SCRIPT_ERR_UNKNOWN_ERROR, 'Script too short for PP1_SP');
    }
    _header = PP1SpHeader.parse(buf);
  }

  PP1SpHeader get header => _header!;

  /// The body after the header, as parsed.
  List<int> get body => script!.buffer.sublist(PP1SpHeader.bytesTotal);
}

/// Locking-script builder for a slot output (verifier or append): a constant
/// script per pool version.
class PP1SpSlotLockBuilder extends LockingScriptBuilder {
  final SVScript _script;
  PP1SpSlotLockBuilder(this._script);
