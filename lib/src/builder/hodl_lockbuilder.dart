...
/// Builds an sCrypt-based HODL locking script that time-locks funds until a specified block height.
///
/// The script enforces that the spending transaction's nLockTime is at or above
/// the configured lock height, preventing the UTXO from being spent before that block.
class HodlLockBuilder extends LockingScriptBuilder{
  ...
  /// Creates a HODL lock builder.
  ///
  /// [_pubKeyHash] - 20-byte pubkey hash of the owner who can spend after the lock expires.
  /// [_lockHeight] - The block height at which the funds become spendable.
  HodlLockBuilder(this._pubKeyHash, this._lockHeight);

  /// Reconstructs a [HodlLockBuilder] by parsing a hex-encoded script.
  HodlLockBuilder.fromHex(String asm) {
    parse(SVScript.fromHex(asm));
  }

  /// The block height at which funds become spendable.
  BigInt? get lockHeight => _lockHeight;

  /// The 20-byte pubkey hash of the owner.
