...
/// public key of the note-encryption KEM whose key pair is derived from
/// (ivk, d), so the viewing key alone opens notes sent to every address
/// of the wallet. KEM 1 is X25519; the X25519 + ML-KEM-768 hybrid the
/// design settles on takes KEM id 2 once ML-KEM lands in the native crate.
class NoteAddress {
  static const kemX25519 = 1;
  final List<int> d;
  final List<int> pkd;
  final int kem;
  final Uint8List epk;
  NoteAddress(this.d, this.pkd, this.kem, this.epk) {
    ...
    if (kem != kemX25519 || epk.length != 32) throw ArgumentError('KEM $kem');
  }
  ...
  static Future<SimpleKeyPair> kemKeyPair(List<int> ivk, List<int> d) => X25519().newKeyPairFromSeed(kemSeed(ivk, d));

  static Future<NoteAddress> derive(List<int> ivk, List<int> d) async {
    final pair = await kemKeyPair(ivk, d);
    final pub = await pair.extractPublicKey();
    return NoteAddress(d, PoolHash.pkdFromIvk(ivk, d), kemX25519, Uint8List.fromList(pub.bytes));
  }

  /// Address [index] of the wallet (see [PoolHash.diversifier]).
  static Future<NoteAddress> at(List<int> ivk, int index) => derive(ivk, PoolHash.diversifier(ivk, index));

  Uint8List get bytes => Uint8List.fromList([kem, ...lanesToBytes(d), ...lanesToBytes(pkd), ...epk]);
  static NoteAddress parse(List<int> b) {
    if (b.length != 1 + 12 + 32 + 32) throw FormatException('address length');
...
class NoteBundle {
  ...
  final List<int> cm;
  final int kem;
  final Uint8List ephemeral; // the sender's KEM public value
  final Uint8List ciphertext;
  final Uint8List outgoing;
  final Uint8List? issuerEphemeral, issuerCopy;
  NoteBundle(this.cm, this.kem, this.ephemeral, this.ciphertext, this.outgoing, {this.issuerEphemeral, this.issuerCopy}) {
    if (cm.length != 8 || kem != NoteAddress.kemX25519 || ephemeral.length != 32) throw ArgumentError('bundle');
    if (ciphertext.length != ctLength || outgoing.length != outLength) throw ArgumentError('bundle lengths');
    if ((issuerEphemeral == null) != (issuerCopy == null)) throw ArgumentError('issuer copy');
    if (issuerCopy != null && (issuerEphemeral!.length != 32 || issuerCopy!.length != outLength)) throw ArgumentError('issuer copy');
  }
  ...
  static const _base = 1 + 32 + 1 + 32 + ctLength + outLength + 1;
  static int lengthOf(List<int> b, int at) {
    if (b.length < at + _base || b[at] != version) throw FormatException('note bundle');
    return _base + (b[at + _base - 1] == 1 ? 32 + outLength : 0);
  }
  ...
  static NoteBundle parse(List<int> b, [int at = 0]) {
    ...
    o += 32;
    final kem = b[o++];
    final eph = Uint8List.fromList(b.sublist(o, o + 32));
    o += 32;
    ...
    Uint8List? ie, ic;
    if (issuer) {
      ie = Uint8List.fromList(b.sublist(o, o + 32));
      o += 32;
      ic = Uint8List.fromList(b.sublist(o, o + outLength));
      o += outLength;
    }
    if (o != at + n) throw FormatException('note bundle');
    return NoteBundle(cm, kem, eph, ct, out, issuerEphemeral: ie, issuerCopy: ic);
  }
...
class NoteEncryption {
  ...
  /// KEM to [epk]: (the sender's public value, the shared secret).
  static Future<(Uint8List, List<int>)> _encaps(Uint8List epk, Random rng) async {
    final seed = Uint8List.fromList(List.generate(32, (_) => rng.nextInt(256)));
    final pair = await _x.newKeyPairFromSeed(seed);
    final pub = await pair.extractPublicKey();
    final ss = await _x.sharedSecretKey(keyPair: pair, remotePublicKey: SimplePublicKey(epk, type: KeyPairType.x25519));
    return (Uint8List.fromList(pub.bytes), await ss.extractBytes());
  }

  static Future<List<int>> _decaps(SimpleKeyPair pair, Uint8List ephemeral) async {
    final ss = await _x.sharedSecretKey(keyPair: pair, remotePublicKey: SimplePublicKey(ephemeral, type: KeyPairType.x25519));
    return ss.extractBytes();
  }

  /// The bundle for [note] sent to [to], with the sender's copy under
  /// [ovk] and, when [issuerEpk] is given, the issuer's copy.
  static Future<NoteBundle> encrypt(NotePlaintext note, NoteAddress to, List<int> ovk, {Uint8List? issuerEpk, Random? rng}) async {
    rng ??= Random.secure();
    final cm = note.cmUnder(to.pkd);
    final cmBytes = lanesToBytes(cm);
    final (eph, secret) = await _encaps(to.epk, rng);
    ...
    Uint8List? ie, ic;
    if (issuerEpk != null) {
      final (ieph, isecret) = await _encaps(issuerEpk, rng);
      ie = ieph;
      ic = await _seal(await _noteKey(isecret, cm, 'issuer'), copy, cmBytes);
    }
    return NoteBundle(cm, to.kem, eph, ct, out, issuerEphemeral: ie, issuerCopy: ic);
  }
  ...
  static Future<NotePlaintext?> decryptIncoming(NoteBundle b, List<int> ivk, List<int> d) async {
    final pair = await NoteAddress.kemKeyPair(ivk, d);
    final secret = await _decaps(pair, b.ephemeral);
  ...
  /// As the asset's issuer, holding the KEM key pair its record's key
  /// refers to.
  static Future<(NotePlaintext, List<int>)?> decryptAsIssuer(NoteBundle b, SimpleKeyPair issuerPair) async {
    if (!b.hasIssuerCopy) return null;
    final secret = await _decaps(issuerPair, b.issuerEphemeral!);
